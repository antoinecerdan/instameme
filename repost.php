...
$stmt = $db->prepare("SELECT chemin_image,description FROM contenus WHERE id = :id");
$stmt->execute([
    "id" => $_GET["memeid"]
]);
...
<body>
    ...
    <div id="content">
        <form id="new-post" enctype="multipart/form-data" method="post">
            <div id="post-top">
                <img class="meme" src="memes/<?= $content[0]["chemin_image"] ?>">
                <p> Description : </p>
                <input type="text" id="desc" name="descr">
            </div>
            <button id="upload">Reposter</button>
        </form>
    </div>
